$(document).ready(function () {
  ...
  var AudioContext =
    ...
    window.webkitAudioContext || //para versiones pasadas
    false;

  if (!AudioContext) {
    alert("lo siento pero debes cambiar a otro navegador");
  } else {
    // inicio--------------------------------------------------------------------
    ...
    errOsc.start(0.0); //tiempo de inicio----------------------------------------------------------
    ...
    var oscillators = frequencies.map(function (frq) {
      ...
      osc.start(0.0); //tiempo de inicio----------------------------------------------------------
      return osc;
    });
    ...
    function setTimeStep(num) {
      ...
      if (num < 4) return tSteps[0];
      if (num < 8) return tSteps[1];
      if (num < 12) return tSteps[2];
      return tSteps[3];
    }
    ...
    $("*").mouseup(function (e) {
      e.stopPropagation();
      if (!gameStatus.lock) stopGoodTones();
    });
